            ...
            for i in first_lines:
                if "_T" in i:
                    ...
                    for cli in CLINET_ARR:
                        thread = threading.Thread(target=handle_ans,
                                                  args=(ans[1], cli))  # creates new thread for client
                        thread.start()
                        arr.append(thread)
                    for thr in arr:
                    ...
                    for i in CLINET_ARR:
                        i.socket.send(build_ans(
                            str(i.value) + "_" + str(i.added_value) + "_" + str(CLINET_ARR.index(i) + 1)).encode())
...
def all_message(sock):  # recievs all of the message based on the message length given at the begining of the messsage
