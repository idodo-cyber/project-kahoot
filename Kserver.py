...
def handleactive(c, addr):  # assigns an active client a personalized id,and closes connection when finished
    ...
    print(ACTIVEREQ)
    txt = all_message(
        c)  # waits until the active client responds(will delete the client from actives even when there is an unexpected disconnection
    if txt == "bye":
        lock.acquire()
...
def all_message(sock):  # recievs all of the message based on the message length given at the begining of the messsage
    ...
    while "_" not in lent:
        lent += sock.recv(1).decode()
    lent = int(lent[:-1])  # receives the message length
